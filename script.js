...
function ready(mode) {
  ...
  const url = 'https://storage.googleapis.com/magentadata/js/checkpoints/music_vae/' +
              (isMelodyMode ? 'mel_4bar' : 'trio_4bar');
  ...
  for (let i = 0; i < N_DIMS; i++) {
    ...
    div.innerHTML = `<input type="range" data-index=${i} min="-2" max="2" step="0.01" value="0">`;
...
function loadFile() {
  ...
  midime = new mm.MidiMe({epochs: 500});
  midime.initialize();
...
async function showInput(ns) {
  ...
  ns.forEach((m) => {
    ...
    instruments.push(i)
    if (i.length > 3) shouldSplit = true;
  });
